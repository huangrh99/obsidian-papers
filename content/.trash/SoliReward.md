## 论文：SoliReward: Mitigating Susceptibility to Reward Hacking and Annotation Noise in Video Generation Reward Models

**arXiv: 2512.22170 | 2025.12**
...
SoliReward 从数据标注（binary classification + cross-prompt pairing）、训练目标（BT-WT loss 正则化正样本分布）、模型架构（HPQA 多层聚合）三个层面系统性解决 reward hacking 和标注噪声问题。核心发现是 RM accuracy 不足以衡量 RM 质量，reward margin 同样关键。BT-WT loss 通过 tie 约束实现隐式正则化，是一种简洁有效的 reward hacking 缓解方案。
